...
---
# Архитектура проекта
В проекте используется архитектурный паттерн MVP (Model-View-Presenter). В соответствии с данным паттерном приложение разделено на три основных слоя: слой данных (Model), слой представления (View), и слой презентера (Presenter). Слои модели и представления не связаны между собой, взаимодействие между этими слоями организовано с помощью слоя презентера. В архитектуре проекта применен событийно-ориентированный подход. 
## Базовый код
### 1. Класс `Api`
Базовый класс-клиент API для взаимодействия с сервером, содержит методы для выполнения HTTP-запросов к серверу и обработки ответа сервера.
...
**Поля**:
- `_counter: HTMLElement` - DOM-элемент счетчика товаров в корзине.
- `_catalog: HTMLElement` - DOM-элемент каталога товаров.
- `_wrapper: HTMLElement` - DOM-элемент, является оберткой контента страницы.
- `_basket: HTMLElement` - DOM-элемент корзины на странице.
...
- `_content: HTMLElement` - DOM-элемент для отображения переданной разметки.
...
- `_price: HTMLElement` - DOM-элемент для отображения цены товара.
- `_image?: HTMLImageElement` - DOM-элемент изображения товара.
...
**Поля**:
- `_list: HTMLElement` - DOM-элемент списка товаров, добавленных в корзину.
- `_total: HTMLElement` - DOM-элемент для отображения общей суммы товаров в корзине.
- `_button: HTMLElement` - DOM-элемент кнопки оформления заказа.

Для хранения списка товаров в корзине и общей стоимости предусмотрены **set-методы**:
...
**Поля**:
- `_cardButton: HTMLButtonElement` - DOM-элемент кнопки оплаты онлайн.
- `_cashButton: HTMLButtonElement` - DOM-элемент кнопки оплаты при получении.
- `_addressInput: HTMLInputElement` - DOM-элемент текстового поля для ввода адреса.
  
Для хранения выбранного способа оплаты и адреса предусмотрены **set-методы**:
...
**Поля**:
- `_phoneInput: HTMLInputElement` - DOM-элемент текстового поля для ввода телефона.
- `_emailInput: HTMLInputElement` - DOM-элемент текстового поля для ввода электронной почты.
  
Для хранения адреса электронной почты и номера телефона предусмотрены **set-методы**:
...
**Поля**:
- `_total: HTMLElement` - DOM-элемент для отображения общей суммы заказа.
- `_close: HTMLButtonElement` - DOM-элемент кнопки возврата к главной странице.

Для хранения общей суммы заказа предусмотрен **set-метод**:
...
1. Пользователь кликнул на кнопку `В корзину` в модальном окне просмотра товара - объект класса `Card` в слое представления генерирует событие `product:add`.
...
5. Перерисовываются компоненты представления `Card`, `Basket`, `Page`.
...
- `contacts:submit` - отправлена форма с контактами (заказ отправлен на сервер).
- `modal:open` - отрыто модальное окно.
- `modal:close` - закрыто модальное окно.

## Основные типы данных
### Тип `Category`
Тип содержит перечень категорий товаров в каталоге.

`type Category = 'софт-скил' | 'другое' | 'дополнительное' | 'кнопка' | 'хард-скил'`.
### Тип `Payment`
Тип описывает возможные способы оплаты заказа.

`type Payment = 'card' | 'cash'`.
### Тип `FormErrors`
Тип данных для ошибок валидации форм.

`type FormErrors = Partial<Record<keyof IOrder, string>>`.
### Интерфейс `IProduct`
Интерфейс описывает свойства объекта товара.
```
interface IProduct {
